...
/**
 * One balanced double-entry transaction in the general ledger, holding its debit and credit lines.
 *
 * <p>The sum of debits across the lines equals the sum of credits. Once POSTED an entry is not
 * edited; a correction is made by posting a reversal, and the pair are linked through
 * {@code reversedByEntryId} on the original and {@code reversesEntryId} on the reversal. When an
 * entry is generated by another document, {@code sourceType} and {@code sourceId} record where it
 * came from (an invoice, a payment, a manual entry, or a reversal).
 */
@Entity
