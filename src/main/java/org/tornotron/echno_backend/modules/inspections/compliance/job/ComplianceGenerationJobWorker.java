...
public class ComplianceGenerationJobWorker {
    ...
    public void run(UUID jobId, String nodeId) {
        ...
        if (!moduleRegistry.isEnabledForOrg(InspectionsModule.ID, orgId)) {
            // A queued row outlives the entitlement it was accepted under. Left RUNNING it
            // would lease-expire and come round again, so it is closed on the row instead.
            log.debug("Compliance generation job {} skipped: the inspections module is not enabled for organization {}",
                    jobId, orgId);
            tenantScopedJobRunner.runForTenant(orgId, () -> writeTerminal(jobId, orgId, nodeId, row.getAttempt(),
                    ComplianceJobStatus.FAILED, 0, "The inspections module is not enabled for this organization."));
            return;
        }
