...
import org.tornotron.echno_backend.modules.inspections.compliance.Jurisdiction;
...
public class ComplianceRuleSweep {
    ...
    void runPass() {
        ...
        log.info("Compliance sweep looked at {} approved project(s): queued {}, {} already running, "
                        + "{} up to date, {} not assessable yet, {} in organizations without the module",
                candidates.size(), enqueued, alreadyRunning, upToDate, notAssessable, notEnabled);
    }
    ...
    private enum Outcome { ENQUEUED, ALREADY_RUNNING, NOT_ASSESSABLE, NOT_ENABLED }
    ...
    private Outcome submit(ComplianceGenerationJobRepository.SweepCandidate candidate) {
        ...
        if (!moduleRegistry.isEnabledForOrg(InspectionsModule.ID, orgId)) {
            log.debug("Compliance sweep skipped project {}: the inspections module is not enabled for organization {}",
                    projectId, orgId);
