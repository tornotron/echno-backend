package org.tornotron.echno_backend.modules.inspections.compliance.job;
...
import org.tornotron.echno_backend.modules.inspections.compliance.ComplianceGenerationProgress;
import org.tornotron.echno_backend.modules.inspections.compliance.ComplianceGenerationService;
...
class ComplianceGenerationJobWorkerTest {
    ...
    void setUp() {
        ...
        worker = new ComplianceGenerationJobWorker(
                ...
                retryTemplate, properties, moduleRegistry);
     ...
     */
    /**
     * A queued row outlives the entitlement it was accepted under. Left alone it would sit
     * RUNNING until its lease expired and be claimed again, so a dark organization's job is
     * closed as failed with the reason on the row, and the model is never called.
     */
    @Test
    void aJobForAnOrganizationWithoutTheModuleIsFailedWithoutRunning() {
        when(moduleRegistry.isEnabledForOrg(InspectionsModule.ID, ORG_ID)).thenReturn(false);
        ...
        assertThat(row.getErrorMessage()).contains("inspections module is not enabled");
        assertThat(row.getFinishedAt()).isNotNull();
    }
