package org.tornotron.echno_backend.modules.inspections.compliance.sweep;
...
class ComplianceRuleSweepTest {
    ...
    /**
     * The entitlement question, asked per project because every project carries its own
     * organization. A tenant whose plan lost the module, or that the operator switched off,
     * has nothing queued on its behalf and the sweep says so only at DEBUG.
     */
    @Test
    void skipsAProjectWhoseOrganizationDoesNotHaveTheModule() {
        catalogueChangedAt(RULE_CHANGED);
        candidates(project(1L, "Tamil Nadu", null));
        ...
        verify(moduleRegistry).isEnabledForOrg(InspectionsModule.ID, ORG_ID);
    }
